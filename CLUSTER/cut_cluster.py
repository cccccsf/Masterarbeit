...
class Cluster(object):
    ...
    def read_Center(self):
        ...
        if isinstance(self.centre_manu, list) and len(self.centre_manu) != 0:
            if isinstance(
                    self.centre_manu[0],
                    list) and len(
                    self.centre_manu) == 2:
    ...
    def get_infos(self):
        ...
        if len(self.basic_infos) != 0:
            ...
            dimensionality, lattice_vector, geometry = read_and_write_infos(
                self.job)
    ...
    def get_center_ma(self):
        ...
        if len(self.centre_manu) == 2:
            if self.UnderCenter_manu is None:
    ...
    def choose_atoms_from_distance(self):
        ...
        original_upper = [
            atom for atom in original_atoms if atom.no in upper_no]
        for atom in original_upper:
            atom.layer = 1
        original_under = [
            atom for atom in original_atoms if atom.no in under_no]
        for atom in original_under:
            atom.layer = 2
        if self.check_factors():
            choosed_atoms = self.select_atoms_according_to_factors(
                original_upper, original_under)
        else:
                ...
                if self.size == 'XS':
                    ...
                    for atom in curr_upper:
                        if self.cal_distance(
                            atom,
                            self.UpperCenter) < self.l * self.zoom and self.cal_vec_distance(
                            atom,
                            self.UpperCenter,
                            self.lattice_vector[1]) < self.l2 * 0.7 * self.zoom * self.fac2 and self.cal_vec_distance(
                            atom,
                            self.UpperCenter,
                                self.lattice_vector[0]) < self.l1 * self.zoom * self.fac1:
                            new_atoms.append(atom)
                    for atom in curr_under:
                        if self.cal_distance(
                            atom,
                            self.UnderCenter) < self.l * self.zoom and self.cal_vec_distance(
                            atom,
                            self.UnderCenter,
                            self.lattice_vector[1]) < self.l2 * 0.7 * self.zoom * self.fac2 and self.cal_vec_distance(
                            atom,
                    ...
                    for atom in curr_under:
                        if self.cal_distance(
                                ...
                                atom,
                                self.UnderCenter,
                                self.lattice_vector[1]) < self.l2 * 0.7 * self.zoom * self.fac2:
                            new_atoms.append(atom)
                elif self.size == 'M':
                    vec_att = (1, 1)
                    curr_atoms = self.gen_all_atoms(original_atoms, vec_att)
                    ...
                    for atom in curr_upper:
                        if self.cal_distance(
                            atom,
                            self.UpperCenter) < self.l * 1.2 * self.zoom and self.cal_vec_distance(
                            atom,
                            self.UpperCenter,
                            self.lattice_vector[1]) < self.l2 * 1.3 * self.zoom * self.fac2 and self.cal_vec_distance(
                            atom,
                            self.UpperCenter,
                                self.lattice_vector[0]) < self.l1 * 1.8 * self.zoom * self.fac1:
                    ...
                    for atom in curr_under:
                        if self.cal_distance(
                            atom,
                            self.UnderCenter) < self.l * 1.2 * self.zoom and self.cal_vec_distance(
                            atom,
                            self.UnderCenter,
                            self.lattice_vector[1]) < self.l2 * 1.3 * self.zoom * self.fac2 and self.cal_vec_distance(
                            atom,
    ...
    def select_atoms_according_to_factors(
            self, original_upper, original_under):
        new_atoms = []
        if self.dimensionality == 2:
            ceil = int(
                math.ceil(max(max(self.factors[0]), max(self.factors[1]))))
            vec_att = (ceil, ceil)
            curr_upper = self.gen_all_atoms(original_upper, vec_att)
            ...
            for atom in curr_upper:
                ...
                if self.cal_distance(atom,
                                     self.UpperCenter) < self.l * fac and self.cal_vec_distance(atom,
                                                                                                self.UpperCenter,
                                                                                                self.lattice_vector[1]) < self.l2 * fac2 and self.cal_vec_distance(atom,
                                                                                                                                                                   self.UpperCenter,
                                                                                                                                                                   self.lattice_vector[0]) < self.l1 * fac1:
                    new_atoms.append(atom)
            fac, fac1, fac2 = self.factors[1]
            for atom in curr_under:
                if self.cal_distance(atom,
                                     self.UnderCenter) < self.l * fac and self.cal_vec_distance(atom,
                                                                                                self.UnderCenter,
                                                                                                self.lattice_vector[1]) < self.l2 * fac2 and self.cal_vec_distance(atom,
                                                                                                                                                                   self.UnderCenter,
                                                                                                                                                                   self.lattice_vector[0]) < self.l1 * fac1:
            ...
            for atom in curr_upper:
                if self.cal_distance(atom,
                                     self.UpperCenter) < self.l * fac and self.cal_vec_distance(atom,
                                                                                                ...
                                                                                                self.lattice_vector[1]) < self.l2 * fac2 and self.cal_vec_distance(atom,
                                                                                                                                                                   self.UpperCenter,
                                                                                                                                                                   self.lattice_vector[0]) < self.l1 * fac1 and self.cal_vector_between_two_atoms(atom,
                                                                                                                                                                                                                                                  self.UpperCenter,
    ...
    def check_factors(self):
        if self.dimensionality == 2:
            if len(self.factors) == 0:
                ...
                fac, fac1, fac2 = self.factors[0]
                if self.check_factor(fac) and self.check_factor(
                        fac1) and self.check_factor(fac2):
                    return True
        elif self.dimensionality == 3:
            if len(self.factors) == 0:
                return False
                ...
                fac, fac1, fac2, fac3 = self.factors[0]
                if self.check_factor(fac) and self.check_factor(
                        fac1) and self.check_factor(fac2) and self.check_factor(fac3):
                    return True
        return False
