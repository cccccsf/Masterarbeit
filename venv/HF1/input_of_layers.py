#!/usr/bin/python3
from HF1.generation_of_input import Gen_Inp
from HF1.generation_of_input import default_basis_set
from copy import deepcopy

class Upper_Layer_Inp(Gen_Inp):

    def __init__(self, path, if_bs_change = 0, ele_to_bs_type=[], elements=[], root_path=''):
        super(Upper_Layer_Inp, self).__init__(path, if_bs_change = 0, ele_to_bs_type=[], elements=[], root_path='')
        self.upperlayer = []
        self.n_fixed_atoms = []
        self.z_fixed_atoms = []
        self.ghost_number = []
    ...
    def get_upperlayer(self):
        count = 1
        for atom in self.geometry:
            if atom[3] >= max(self.z_fixed_atoms) or abs(atom[3] - max(self.z_fixed_atoms)) <= 0.1:
                self.upperlayer.append(atom)
            else:
    ...
    def write_upperlayer_inp_init(self):
        ...
        self.z_dirname = self.z_dirname + '/upperlayer'
