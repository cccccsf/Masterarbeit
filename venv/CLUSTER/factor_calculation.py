...
class FactorCalculator(object):
    ...
    def get_distance_to_center(self, atom, factor=False):
    ...
    def get_distance_to_vector(self, atom, vec, factor=False)
        ...
        if type(vec) == tuple and len(vec) == 3:
            ...
            if vec == 'a' or vec == 'x' or int(vec) == 1:
                vx, vy, vz = self.lattice_vector[0]
                l = self.l1
            elif vec == 'b' or vec == 'y' or int(vec) == 2:
                vx, vy, vz = self.lattice_vector[1]
                l = self.l2
            elif vec == 'c' or vec == 'z' or int(vec) == 3:
                vx, vy, vz = self.lattice_vector[2]
                l = self.l3
